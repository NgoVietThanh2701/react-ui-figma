...
const CardServices = ({ title_1, title_2, img, bg_text, bg }: Props) => {
  return (
    <div
      className={`flex justify-between text-center p-[50px] gap-[40px] bg-[${bg}] border border-[#191A23]
    shadow-[0_5px_0px_#191A23] rounded-[45px]`}
    >
      <div className="flex flex-col justify-center items-start gap-[93px]">
        <div className="flex flex-col items-start">
          <span
            className={`bg-[${bg_text}] px-[7px] rounded-[7px] font-medium text-3xl leading-[38px]`}
          >
            {title_1}
          </span>
          <span
        ...
        <a href="#" className="flex items-center gap-[15px]">
          ...
          >
            ...
            <path
              ...
              fill={`${bg === '#191A23' ? '#191A23' : '#B9FF66'} `}
            />
          </svg>
          <span
            className={`text-xl leading-7 text-${
              bg === '#191A23' ? 'white' : 'black'
            }`}
          >
            Learn more
          </span>
